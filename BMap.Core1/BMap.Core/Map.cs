...
namespace BMap.Core
{
    public class Map
    {
        ...
        {
            var centerOffSet = new SizeF((float)(mp.X - ScreenArea.Center.X), (float)(mp.Y - ScreenArea.Center.Y));
        ...
        public void UpdateDrawTiles()
        {
            ...
            try
            {
                ...
                if (ScreenArea != null)
                {
                    ...
                    var pointLeftTopPix = MapCenterPix.GetOffSet(-width / 2, -height / 2);//左上角像素坐标
                    ...
                    for (int x = 0; x < xCount; x++)
                    {
                        for (int y = 0; y < yCount; y++)
                        {
                            ...
                            //if (RotationAngle == 0 || MapHelper.CheckTileCrossRect(new RectInt(dt.DrawPosition.GetOffSet(TilesArea.LeftTop.X, TilesArea.LeftTop.Y), TileSize.Width, TileSize.Height), ScreenArea, RotationAngle))
                            {
                                ...
                                if (t.ImgData == null || t.ImgData.Length == 0)
                                {
                                    TileManager.Singleton.LoadTile(t);
                                }
                            }
